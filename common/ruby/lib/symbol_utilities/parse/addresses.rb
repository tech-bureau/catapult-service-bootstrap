...
module SymbolUtilities
  module Parse
    module Addresses
      ...
      SECTION_SIZES = {
        Global::ParseKey.peer_nodes => 2,
      }
      
      ADDRESS_TYPES = [:private, :public, :address]
      Element = Struct.new(*ADDRESS_TYPES)
      def self.parse(raw_address_info, break_into_sections: false)
        parsed_flat_form = parse_into_flat_form(raw_address_info)
        break_into_sections ? break_into_sections(parsed_flat_form) : parsed_flat_form
      end
      
      private
      
      def self.parse_into_flat_form(raw_address_info)
        parsed_form = []
        next_state   = :private # states can be :private, :public, :address
        next_element = {}
        raw_address_info.each_line do |line|
